...
def test_humanize():
    ...
    cases = [
        ...
        ("Является эффективным", "является эффективным"),  # не должно быть замены в середине
        ("В заключение", "Короче"),
...
def test_system_prompt_quality():
    ...
    ai_flags = ["уникальный", "инновационный", "революционный",
                "в современном мире", "следует отметить"]

    for flag in ai_flags:
        assert flag not in SYSTEM_PROMPT.lower(), (
            f"Системный промпт содержит AI-штами '{flag}'!"
        )
    print("  ✓ Системный промпт чист от AI-штампов")
