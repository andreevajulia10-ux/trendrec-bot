...
def get_or_create_user(tg_id: int, username: str | None, first_name: str | None, last_name: str | None) -> dict:
    conn = get_connection()
    cur = conn.execute(
...
def get_user_niches(tg_id: int) -> list[dict]:
    conn = get_connection()
    rows = conn.execute(
...
def set_user_niches(tg_id: int, niche_ids: list[int]) -> None:
    conn = get_connection()
...
def get_trends_for_niches(niche_ids: list[int], limit: int = 5) -> list[dict]:
    if not niche_ids:
        return []
    conn = get_connection()
...
def get_daily_trend(niche_ids: list[int]) -> dict | None:
    if not niche_ids:
        return None
    conn = get_connection()
    placeholders = ",".join("?" for _ in niche_ids)
...
def get_idea_for_niche(niche_slug: str) -> dict | None:
    conn = get_connection()
    row = conn.execute(
